...
def do_plain_scatter(risk, hazard, exposure):
    plt.subplot(1, 2, 1)
    plt.title("Exposure (Classified) vs Hazard in natural scale", size=20)
    plt.scatter(exposure, hazard, s=8, c="darkblue", edgecolors='none')
    plt.xlabel('Exposure (Classified)', size=16)
    plt.ylabel('Hazard', size=16)
    plt.grid(True)

    plt.subplot(1, 2, 2)
    plt.title("Exposure (Classified) vs RiskTR in natural scale", size=20)
    plt.scatter(exposure, risk, s=8, c="darkblue", edgecolors='none')
    plt.xlabel('Exposure (Classified)', size=16)
    plt.ylabel('RiskTR', size=16)
    plt.grid(True)
...
# side effects with the existence of -99. I will also get rid of the -99
...
path_exp_out_tif = r"D:\PycharmProjects\IGM_PhD_Materials\data\P03\out\Exposure_RD_classified.tif"
...
print(np.unique(tif, return_counts=True))
