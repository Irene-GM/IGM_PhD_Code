...
def write_tif(placed_list, nt, ns):
    ...
    # Get the origin coordinates for the pred_tif file
    geotransform = tif_template.GetGeoTransform()

    i = 0
    for myarr in placed_list:
        path_out = r"D:/PycharmProjects/IGM_PhD_Materials/data/P04/out/pred_tif/{0}.tif".format(names[i].format(ns, nt))
        outDs = tif_template.GetDriver().Create(path_out, rows, cols, 1, gdal.GDT_Float32)
...
def place(stack):
    tif = gdal.Open(r"D:/PycharmProjects/IGM_PhD_Materials/data/data/2014_tmax.tif")
...
nt = [10, 20, 50]
ns = range(100, 900, 100)

path_tmp_in = r"D:/PycharmProjects/IGM_PhD_Materials/data/P04/out/pred_csv/prediction_nl_four_models_{0}T_{1}S.csv"

for n_esti in nt:
    for spl in ns:
        print("Creating TIFF for ensemble ({0} T, {1} SPL)".format(n_esti, spl))
        path_in = path_tmp_in.format(n_esti, spl)
        stack = np.loadtxt(path_in, delimiter=";")
        placed_list = place(stack)
        write_tif(placed_list, n_esti, spl)
